...
use raiko_lib::{
    input::{
        AggregationGuestInput, GuestBatchInput, GuestInput, RawAggregationGuestInput,
...
use crate::{
    ...
    TdxConfig, TDX_AGGREGATION_PROOF_SIZE, TDX_PROOF_SIZE, TDX_SOCKET_PATH,
};
...
pub fn generate_tdx_quote(user_report_data: &B256) -> Result<(Vec<u8>, Vec<u8>)> {
    let user_data = user_report_data.as_slice();
    let nonce: [u8; 32] = rand::thread_rng().gen();
    let nonce = nonce.to_vec();

    info!("Using external attestation service at: {}", TDX_SOCKET_PATH);

    let attestation_doc =
        attestation_client::issue_attestation(TDX_SOCKET_PATH, user_data, &nonce)?;

    Ok((attestation_doc, nonce))
}

pub fn generate_tdx_quote_from_public_key(public_key: &Address) -> Result<(Vec<u8>, Vec<u8>)> {
...
pub fn prove(input: &GuestInput, tdx_config: &TdxConfig) -> Result<ProveData> {
    ...
    let (quote, nonce) = generate_tdx_quote(&pi_hash)?;
...
pub fn prove_aggregation(input: &AggregationGuestInput) -> Result<ProveAggregationData> {
    let private_key = load_private_key()?;
        ...
        proofs: input
            ...
            .map(|proof| {
                Ok(raiko_lib::input::RawProof {
                    ...
                    proof: hex::decode(&proof.clone().proof.unwrap().trim_start_matches("0x"))
                        .unwrap(),
                })
            })
    ...
    let proof =
        TdxAggregationProof::new(instance_id, &old_instance, &new_instance, &signature).to_vec();
    let (quote, nonce) = generate_tdx_quote(&aggregation_hash)?;
...
pub fn prove_shasta_aggregation(
    ...
    let first_proof = TdxProof::from_bytes(&input.proofs[0].proof)?;
    let instance_id = first_proof.instance_id();
    ...
    if !validate_shasta_aggregate_proof_carry_data(&input) {
        return Err(anyhow!("Invalid shasta proof carry data for aggregation"));
    }

    // Verify the proofs
    for (i, proof) in input.proofs.iter().enumerate() {
        // Verify proof input matches the hashed carry data
        if proof.input != hash_shasta_subproof_input(&input.proof_carry_data_vec[i]) {
            return Err(anyhow!(
                "Proof {} input does not match hashed carry data",
                i
            ));
        }
        ...
        // Verify instance matches expected (no rotation allowed)
        if instance != expected_instance {
            return Err(anyhow!(
                "Shasta aggregation does not allow key rotation: proof {} has instance {}, expected {}",
                i,
                instance,
                expected_instance
            ));
        ...
        // Verify signature was created by this instance
        let recovered = recover_signer_unchecked(&signature, &proof.input)?;
        if recovered != expected_instance {
            return Err(anyhow!(
                "Proof {} signature verification failed: expected signer {}, got {}",
                i,
                expected_instance,
                recovered
            ));
