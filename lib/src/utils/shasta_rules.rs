...
pub const ANCHOR_MAX_OFFSET: usize = 256;
...
pub(crate) fn validate_input_block_param(
    ...
    if manifest_block.gas_limit + ANCHOR_V3_V4_GAS_LIMIT != input_block.header.gas_limit {
        warn!(
...
pub fn validate_shasta_block_gas_limit(
    ...
    let mut parent_gas_limit = if block_guest_inputs[0].parent_header.number == 0 {
        ...
        block_guest_inputs[0].parent_header.gas_limit - ANCHOR_V3_V4_GAS_LIMIT
    };
...
mod tests {
    ...
    fn test_timestamp_validation_enforces_shasta_fork_time() {
        ...
        chain_spec.hard_forks =
            BTreeMap::from([(TaikoSpecId::SHASTA, ForkCondition::Timestamp(150))]);
        let proposal = Proposal {
            timestamp: Uint::from(200),
            originBlockNumber: Uint::from(100),
