use alloy_primitives::{Address, ChainId, Uint, B256};
...
pub trait Prover {
    ...
    async fn proposal_run(
        ...
        let last_checkpoint = Checkpoint {
            blockNumber: Uint::from(input.inputs.last().unwrap().block.number),
        ...
        proof.extra_data = Some(ProofCarryData {
            ...
            transition_input: TransitionInputData {
                ...
                parent_checkpoint_hash: hash_checkpoint(&Checkpoint {
                    blockNumber: Uint::from(input.inputs.first().unwrap().parent_header.number),
