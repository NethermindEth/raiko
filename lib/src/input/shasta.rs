...
impl ShastaEventData {
    ...
    fn _decode_event_data_with_abi(data: &[u8]) -> Result<Self, alloy_sol_types::Error> {
        let payload = ProposedEventPayload::abi_decode_validate(data)?;
        Ok(Self {
    ...
    fn decode_event_data(data: &[u8]) -> Result<Self, alloy_sol_types::Error> {
        ...
        for _ in 0..sources_length {
            ...
            sources.push(DerivationSource {
                ...
                blobSlice: BlobSlice {
                    ...
                    timestamp: blob_timestamp.try_into().unwrap(),
                },
            });
        }
        ...
        Ok(Self {
            ...
            derivation: Derivation {
                originBlockNumber: origin_block_number.try_into().unwrap(),
                originBlockHash: origin_block_hash,
                basefeeSharingPctg: basefee_sharing_pctg,
                sources,
            },
            core_state: CoreState {
                nextProposalId: next_proposal_id.try_into().unwrap(),
                lastProposalBlockId: last_proposal_block_id.try_into().unwrap(),
                lastFinalizedProposalId: last_finalized_proposal_id.try_into().unwrap(),
                lastCheckpointTimestamp: last_checkpoint_timestamp.try_into().unwrap(),
